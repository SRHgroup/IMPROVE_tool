...
def readNetOutDirApply(inDir,cond = 'eval',header=False,cols=[],skiprows=50,skipfooter=5):
    ...
    for filename in os.listdir(inDir):
        if filename.startswith(cond):
            try:
                df = NNanalysis.NetMHCIIpanOutput2DF(inDir,filename,skiprows=skiprows,skipfooter=skipfooter,header=header)
            except:
                print(filename)
                continue
...
def readNetMHCpan41(inDir,filename,header=False,skiprows=50,skipfooter=5):
    return NNanalysis.NetMHCIIpanOutput2DF(inDir,filename,skiprows=skiprows,skipfooter=skipfooter,header=header)
...
def runNetMHCpan(inDir,outDir=False,inFile='tempPept.txt',outFile='logoFiles',clean=False,printCMD=False,srcPath=False,**kwargs):
    if not srcPath:
        CMDPath = '/Users/birey/Dropbox/2018_TCell_PhD/code/CBS-programs/netMHCIIpan-3.2/netMHCIIpan'
    else:
        CMDPath = srcPath
    if not outDir:
        outDir = inDir
